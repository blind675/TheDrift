...
function DriftScreen({ categories, entries, shares, windowDays, setWindowDays, category }: any) {
  ...
  const logged = visible.reduce((n: number, e: Entry) => n + minutes(e), 0);
  const maintenanceId = category("maintenance")?.id;
  const maint = maintenanceId ? totals[maintenanceId] || 0 : 0;
  const coverageDays = windowDays === 0 ? 1 : Math.min(windowDays, averagingDays);
  const coverage = Math.min(100, Math.round(logged / (coverageDays * 960) * 100));
  const coverageMessage = coverage < 25
    ? `You have logged ${coverage}% of an assumed 16-hour waking day. With partial logs, rank movement is more reliable than exact hour comparisons.`
    : coverage < 70
      ? `You have logged ${coverage}% of an assumed 16-hour waking day. Share gaps are useful, but some time is still untracked.`
      : `You have logged ${coverage}% of an assumed 16-hour waking day, giving this comparison strong coverage.`;
